...
const registerUser= asyncHandler(async(req,res)=>{
    ...
    //checking if user exists or not if exists do no proceed further
    if(existedUser){
...
const verifyEmail=asyncHandler(async(req,res)=>{
        ...
        const user= await User.findOne({
            ...
            emailVerificatonExpiry: {$gt:Date.now()}
        })
    ...
    await sendEmail(
        ...
        .json(
            new ApiResponse(
                200,
                {},
                "Password reset mail has been sent on your email or mail-id"
            )
        )
})
